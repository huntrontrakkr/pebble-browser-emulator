...
| MPU access enforcement     | Focused PMSAv7/M4 and PMSAv8/M33 tests verify a denied store enters MemManage, latches DACCVIOL/MMARVALID and leaves backing RAM unchanged. Unchanged Sports/Gabbro now matches native QEMU's stack-guard result. This is focused behavior, not complete MPU or security conformance.                                                                                         |
...
- **MPU, privilege and security:** Ordinary instruction and data paths now enforce PMSAv7 for Flint and PMSAv8 for Emery/Gabbro, including privilege, read-only, execute-never and enabled region bounds. Overlap edge cases, all fault-priority combinations, SAU attribution, Secure/Non-secure transitions and banked exception behavior still need an independent architecture oracle. Do not advertise complete MPU isolation or TrustZone support.
...
With the delivered patches, 48 tests pass in the focused set: `wakeup_exception.rs` (16), `priority_bytes.rs` (4), `exclusive_context.rs` (8), `special_stack_reads.rs` (3), `wfi_pending.rs` (10), `cps_masks.rs` (4), and `mpu_access.rs` (3). All are synthetic vectors with no SDK/firmware dependency. The WFI set had seven failures before its patch; the priority-byte set had four failures before its patch.

```sh
cargo test -p emulator-qemu --release --test wakeup_exception --test priority_bytes --test exclusive_context --test special_stack_reads --test wfi_pending --test cps_masks --test mpu_access
```
