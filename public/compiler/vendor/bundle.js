...
function instantiate(getCoreModule, imports, instantiateCore = WebAssembly.instantiate) {
  ...
  let gen = (function* init() {
    ...
    function trampoline27(arg0, arg1, arg2, arg3, arg4, arg5, arg6, arg7) {
      ...
      if (!rsc5) {
        rsc5 = Object.create(Descriptor3.prototype);
    ...
    function trampoline30(arg0, arg1, arg2, arg3, arg4, arg5, arg6) {
      ...
      if (!rsc4) {
        rsc4 = Object.create(Descriptor3.prototype);
...
// lib/api.js
var ARGV0 = "yowasp-llvm";
var sysroot = "/usr";
var DRIVERS = /* @__PURE__ */ new Set(["clang", "clang++"]);
var assetLoader = null;
function setAssetLoader(loader) {
  assetLoader = loader;
}
function suppliedBy(loader, fallback) {
  return async (url, init) => {
    ...
    if (supplied === void 0 || supplied === null) return fallback(url, init);
...
var modulesOnce = null;
var filesystemOnce = null;
var withLoader = (fetch3) => assetLoader ? suppliedBy(assetLoader, fetch3) : fetch3;
var retryable = (promise, forget) => promise.catch((error) => {
  forget();
  throw error;
});
var resources = {
  totalSize,
  modules: (fetch3) => modulesOnce ??= retryable(modules(withLoader(fetch3)), () => {
    modulesOnce = null;
  }),
  filesystem: (fetch3) => filesystemOnce ??= retryable(filesystem(withLoader(fetch3)), () => {
    filesystemOnce = null;
  })
};
var { Descriptor: Descriptor2 } = new Environment().exports.fs;
if (/not implemented/.test(Descriptor2.prototype.write.toString())) {
  Descriptor2.prototype.write = function(buffer, offset) {
    if (this.entry.data === void 0)
      throw "is-directory";
    this.writeViaStream(offset).write(buffer);
    return BigInt(buffer.length);
  };
}
var llvm = new Application(resources, instantiate, ARGV0);
...
function parseDriverCommands(output) {
  ...
  for (const line of output.split("\n")) {
    ...
    if (line.startsWith(' "')) {
      ...
      commands2.push(argv[0] === "" ? argv.slice(1) : argv);
    } else if (commands2.length === 0) {
      continue;
    } else if (line === "") {
      break;
    } else {
  ...
  return commands2.length === 0 ? null : commands2;
}
function runClang(args = null, files = {}, options = {}) {
  ...
  let gen = (function* () {
    ...
    const commands2 = parseDriverCommands(output);
    if (commands2 === null) {
...
var Session = class {
  ...
  async #exec(argv, { stdin = null, stdout = null, stderr = null } = {}) {
    ...
    const modules2 = await resources.modules();
    const command = await instantiate((name) => modules2[name], { runtime: environment.exports });
    try {
      command.run.run();
      return 0;
    } catch (error) {
  ...
  async #clang(argv, options) {
    ...
    const chunks = [];
    const capture = (bytes) => {
      if (bytes !== null) chunks.push(Uint8Array.from(bytes));
    };
    const code = await this.#exec([tool, "-###", ...args], { stdout: capture, stderr: capture });
    const output = new TextDecoder().decode(concat(chunks));
    const commands2 = code === 0 ? parseDriverCommands(output) : null;
    if (commands2 === null)
      return this.#exec(argv, options);
    if (args.includes("-v"))
      options.stderr?.(new TextEncoder().encode(output));
    const named = new Set(argv);
    const scratch = commands2.flat().filter((arg) => /^\/?tmp\//.test(arg) && !named.has(arg));
    const { root } = await this.#ready();
    const hadTmp = root.files.tmp !== void 0;
    try {
      for (const command of commands2) {
...
function merge(into, from) {
  for (const [name, entry] of Object.entries(from.files)) {
    if (entry.files !== void 0 && into.files[name]?.files !== void 0)
...
var commands = {
  ...
  "ld.lld": subcommand(runLLVM, "ld.lld"),
  // ELF flavour of lld, which is what an Arm target links with
  "clang": subcommand(runClang, "clang"),
  "clang++": subcommand(runClang, "clang++")
};
var version = "21.11.0-alpha.1";
export {
  Exit,
  commands,
  createSession,
