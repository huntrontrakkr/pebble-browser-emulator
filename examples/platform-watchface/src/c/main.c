...
static void load(Window *window) {
  ...
  s_time_layer = make_text(root, GRect(0, bounds.size.h / 2 - 65, bounds.size.w, 58), FONT_KEY_BITHAM_42_BOLD);
  s_device_layer = make_text(root, GRect(18, bounds.size.h / 2 - 8, bounds.size.w - 36, 40), FONT_KEY_GOTHIC_18);
  s_status_layer = make_text(root, GRect(18, bounds.size.h / 2 + 33, bounds.size.w - 36, 44), FONT_KEY_GOTHIC_18);
  size_t length = resource_load(resource_get_handle(RESOURCE_ID_START_MESSAGE),
                                (uint8_t *)s_status, sizeof(s_status) - 1);
  s_status[length] = '\0';
  text_layer_set_text(s_status_layer, s_status);
