...
// does the same for Okio's FileSystem.SYSTEM and openZip (util/PlatformFileSystem.kt).
{
  ...
  const browserOwn = [
    ...
    // Round 39: the PebbleKit JS runner (none on desktop).
    'io/rebble/libpebblecommon/di/PKJSModule.jvm.kt',
  ];
...
`,
  ...
  // Round 39: PebbleKit JS follows upstream's iOS runner. Its dispatch tables for the
  // Pebble, private and geolocation interfaces are plain Kotlin and come from the
  // checkout unchanged; startup.js and the timer shim are the scripts upstream ships.
  const ios = join(dir, 'libpebble3/src/iosMain/kotlin/io/rebble/libpebblecommon/js');
  const browserJs = join(dir, 'libpebble3/src/wasmJsMain/kotlin/io/rebble/libpebblecommon/js');
  for (const file of ['JSCPKJSInterface.kt', 'JSCPrivatePKJSInterface.kt', 'JSCGeolocationInterface.kt'])
    await cp(join(ios, file), join(browserJs, file));
  const kotlinString = (text) =>
    JSON.stringify(text)
      .replace(/\\(.)/g, (escape, c) => (c === 'f' ? '\\u000c' : escape))
      .replace(/\$/g, '\\$');
  const startup = await readFile(join(dir, 'libpebble3/src/androidMain/assets/startup.js'), 'utf8');
  const timeout = await readFile(join(ios, 'JSTimeout.js'), 'utf8');
  // Round 44: the WebSocket class apps see is upstream's iOS one, unchanged.
  const webSocket = await readFile(join(ios, 'WebSocket.js'), 'utf8');
  await writeFile(
    join(browserJs, 'PkjsScripts.kt'),
    `// Generated by tools/phone-spike/patch.mjs from upstream's startup.js, JSTimeout.js and WebSocket.js.
package io.rebble.libpebblecommon.js

internal const val PKJS_STARTUP_JS: String = ${kotlinString(startup)}

internal const val PKJS_TIMEOUT_JS: String = ${kotlinString(timeout)}

internal const val PKJS_WEBSOCKET_JS: String = ${kotlinString(webSocket)}
`,
  );
}
